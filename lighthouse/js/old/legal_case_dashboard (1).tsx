...
const LegalCaseDashboard = () => {
  ...
  // Filtered Data
  const upcomingCourtDates = dummyData.courtDates
    .filter(date => {
      const daysUntil = Math.ceil((new Date(date.date) - today) / (1000 * 60 * 60 * 24));
      return daysUntil >= 0 && daysUntil <= 14;
    })
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  const overdueDocuments = dummyData.documentRequests.filter(doc => 
    new Date(doc.responseDeadline) < today && doc.status !== 'Received'
  );

  const urgentDocuments = dummyData.documentRequests.filter(doc => {
    const daysUntil = Math.ceil((new Date(doc.responseDeadline) - today) / (1000 * 60 * 60 * 24));
  ...
  const StatCard = ({ icon: Icon, label, value, color }) => (
    <div className={`bg-white rounded-lg shadow-sm p-4 border-l-4 ${color}`}>
      <div className="flex items-center justify-between">
        ...
        <Icon className={`w-8 h-8 ${color.replace('border-', 'text-')} opacity-80`} />
      </div>
    </div>
  );

  const DocumentCard = ({ doc, borderColor, bgColor }) => {
    const daysUntil = getDaysUntil(doc.responseDeadline);
    return (
      <div className={`border-l-3 ${borderColor} ${bgColor} p-3 rounded text-sm`}>
        <div className="flex justify-between items-start gap-2">
          ...
          <div className="text-right shrink-0">
            <p className="text-xs font-semibold whitespace-nowrap">{typeof daysUntil === 'string' && daysUntil.includes('days') ? daysUntil : formatDate(doc.responseDeadline)}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className={`border-l-3 p-3 rounded text-sm ${color.border} ${color.bg}`}>
        <div className="flex justify-between items-start gap-2">
          ...
          <div className="text-right shrink-0">
            ...
            <p className={`text-xs font-semibold mt-0.5 ${isUrgent ? 'text-red-600' : 'text-gray-600'}`}>{daysUntil}</p>
          </div>
        </div>
      </div>
    );
  ...
  const navItems = [
    ...
    { id: 'progress', label: 'Progress & Dependencies', icon: AlertTriangle },
    ...
    <div className="min-h-screen bg-gray-50">
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-3 font-medium text-sm transition-colors ${
                    activeView === item.id ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600 hover:text-gray-900'
                  }`}
                >
      ...
      <main className="max-w-[1800px] mx-auto px-8 py-6">
        ...
        {activeView === 'dashboard' && (
          <div className="space-y-4">
            <div className="grid grid-cols-4 gap-4">
              <StatCard icon={Briefcase} label="Active Cases" value={dummyData.cases.length} color="border-blue-500" />
              <StatCard icon={AlertCircle} label="Overdue Docs" value={overdueDocuments.length} color="border-red-500" />
              <StatCard icon={Calendar} label="Court (14d)" value={upcomingCourtDates.length} color="border-green-500" />
              <StatCard icon={AlertTriangle} label="High Priority" value={highPriorityCases.length} color="border-orange-500" />
            </div>
                ...
                <div className="p-3 max-h-96 overflow-y-auto">
                  {overdueDocuments.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {overdueDocuments.map(doc => <DocumentCard key={doc.id} doc={doc} borderColor="border-red-500" bgColor="bg-red-50" />)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-3 max-h-96 overflow-y-auto">
                  {urgentDocuments.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {urgentDocuments.map(doc => <DocumentCard key={doc.id} doc={doc} borderColor="border-orange-500" bgColor="bg-orange-50" />)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="p-3 max-h-96 overflow-y-auto">
                  {upcomingCourtDates.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {upcomingCourtDates.map(event => <CourtEventCard key={event.id} event={event} />)}
                    </div>
                  )}
                </div>
              </div>
                    ...
                    {highPriorityCases.map(caseItem => (
                      <tr key={caseItem.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2">
                          <span className="text-xs font-semibold px-2 py-0.5 rounded bg-blue-100 text-blue-800">{caseItem.status}</span>
                        </td>
                        ...
                        <td className="px-4 py-2">
                          <button 
                            onClick={() => { setSelectedCase(caseItem); setActiveView('case-detail'); }}
                ...
                <tbody className="divide-y divide-gray-200">
                  {dummyData.cases.map(caseItem => (
                    <tr key={caseItem.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs font-semibold px-2 py-1 rounded bg-blue-100 text-blue-800">{caseItem.status}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span className={`font-semibold ${getPriorityColor(caseItem.priority)}`}>{caseItem.priority}</span>
                      </td>
                      <td className="px-4 py-3 text-gray-900">{caseItem.nextCourtDate ? formatDate(caseItem.nextCourtDate) : '-'}</td>
                      <td className="px-4 py-3 text-gray-900">{caseItem.trialDate ? formatDate(caseItem.trialDate) : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {activeView === 'case-detail' && selectedCase && (
          <div>
            <button onClick={() => setActiveView('cases')} className="mb-4 text-blue-600 hover:text-blue-800 font-medium flex items-center gap-1 text-sm">
              ← Back to Cases
            </button>
            <div className="bg-white rounded-lg shadow-sm border border-gray-200">
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-6 mb-6">
                  ...
                  <div>
                    ...
                    <p className={`mt-1 font-semibold ${getPriorityColor(selectedCase.priority)}`}>{selectedCase.priority}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 mt-1">{selectedCase.nextCourtDate ? formatDate(selectedCase.nextCourtDate) : 'Not scheduled'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-gray-700">Trial Date</p>
                    <p className="text-gray-900 mt-1">{selectedCase.trialDate ? formatDate(selectedCase.trialDate) : 'Not scheduled'}</p>
                  </div>
                </div>
                        ...
                        <div key={doc.id} className="border border-gray-200 rounded p-4">
                          <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                              <span className={`text-xs font-semibold px-2 py-1 rounded ${getStatusColor(doc.status)}`}>{doc.status}</span>
                ...
                <tbody className="divide-y divide-gray-200">
                  {dummyData.documentRequests.map(doc => (
                    <tr key={doc.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`font-semibold ${getPriorityColor(doc.priority)}`}>{doc.priority}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              <div className="p-4">
                <div className="grid grid-cols-7 gap-0">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                    <div key={day} className="text-center font-semibold text-sm text-gray-700 py-2 border border-gray-200 bg-gray-50">{day}</div>
                  ))}
                  {renderCalendar()}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  {dummyData.courtDates
                    ...
                    .map(event => {
                      ...
                      return (
                        <div key={event.id} className={`border-l-4 p-4 rounded ${color.border} ${color.bg}`}>
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex items-center gap-3">
                                <p className="font-semibold text-gray-900">{event.type}</p>
                                <span className={`text-xs font-semibold px-2 py-1 rounded ${isUrgent ? 'bg-red-200 text-red-800' : 'bg-gray-200 text-gray-800'}`}>
                                  {daysUntil}
                                </span>
                              </div>
              ...
              <div className="space-y-6">
                {dummyData.cases.map(caseItem => {
                  ...
                  const milestones = [
                    ...
                    { id: 'trial', label: 'Trial', completed: Boolean(caseItem.trialDate && new Date(caseItem.trialDate) < today), blocked: caseItem.trialDate === null }
                  ];
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-2">
                            ...
                            <span className="text-xs font-semibold px-2 py-1 rounded bg-white text-gray-700">{caseItem.status}</span>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <div key={milestone.id} className="flex items-start gap-3">
                            <div className="flex flex-col items-center">
                              <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 ${
                                milestone.completed ? 'bg-green-500 border-green-500' : milestone.blocked ? 'bg-gray-300 border-gray-300' : 'bg-white border-blue-500'
                              }`}>
                                {milestone.completed && <CheckCircle className="w-4 h-4 text-white" />}
                                {milestone.blocked && <Clock className="w-4 h-4 text-gray-600" />}
                              </div>
                              {idx < milestones.length - 1 && <div className={`w-0.5 h-8 ${milestone.completed ? 'bg-green-500' : 'bg-gray-300'}`}></div>}
                            </div>
                            <div className="flex-1 pt-0.5">
                              <div className="flex items-center justify-between">
                                <p className={`font-semibold text-sm ${milestone.completed ? 'text-green-700' : milestone.blocked ? 'text-gray-500' : 'text-gray-900'}`}>
                                  {milestone.label}
                                ...
                                <div className="mt-2 text-xs bg-yellow-50 border border-yellow-200 rounded p-2">
                                  ...
                                  <ul className="mt-1 space-y-0.5 text-yellow-700">
                                    {dummyData.documentRequests
                                      ...
                                      .slice(0, 3)
                                      .map(doc => <li key={doc.id}>• {doc.type} from {doc.recipient} - {doc.status}</li>)}
                                  </ul>
                                </div>
                              )}
                  ...
                  {dummyData.cases
                    ...
                    .map(caseItem => (
                      ...
                      >
                        ...
                        {caseItem.trialDate && <p className="text-xs text-blue-600 font-semibold mt-1">Trial: {formatDate(caseItem.trialDate)}</p>}
                      </button>
                    ))}
                </div>
              </div>
                ...
                <div className="p-6 max-w-4xl mx-auto space-y-6">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div><span className="font-semibold">Information Number:</span><span className="ml-2 text-gray-700">[To be filled]</span></div>
                      <div><span className="font-semibold">Accused:</span><span className="ml-2 text-gray-700">{selectedCase.accused}</span></div>
                      <div><span className="font-semibold">DOB:</span><span className="ml-2 text-gray-700">{selectedCase.dob}</span></div>
                      <div><span className="font-semibold">Charge(s):</span><span className="ml-2 text-gray-700">{selectedCase.charges}</span></div>
                      <div><span className="font-semibold">Offence Date:</span><span className="ml-2 text-gray-700">{formatDate(selectedCase.offenceDate)}</span></div>
                      <div><span className="font-semibold">Location:</span><span className="ml-2 text-gray-700">{selectedCase.location}</span></div>
                      <div><span className="font-semibold">Investigator:</span><span className="ml-2 text-gray-700">{selectedCase.investigator}</span></div>
                      <div><span className="font-semibold">Detachment:</span><span className="ml-2 text-gray-700">{selectedCase.detachment}</span></div>
                      <div className="col-span-2"><span className="font-semibold">Criminal Record:</span><span className="ml-2 text-gray-700">{selectedCase.criminalRecord}</span></div>
                    </div>
                  </div>
